...
from midas.fit import ssr, jacobian, ssr2, jacobian2
...
def estimate(y, yl, x, poly="beta"):
    ...
    if yl is not None:
        c = np.linalg.lstsq(
            np.concatenate(
                [np.ones((len(xw), 1)), xw.reshape((len(xw), 1)), yl], axis=1
            ),
            y,
        )[0]
        ...
        c = np.linalg.lstsq(
            np.concatenate([np.ones((len(xw), 1)), xw.reshape((len(xw), 1))], axis=1), y
        )[0]
    ...
    if yl is not None:
        c = np.linalg.lstsq(
            np.concatenate(
                [
                    np.ones((len(xw1), 1)),
                    xw1.reshape((len(xw1), 1)),
                    xw2.reshape((len(xw2), 1)),
                    yl,
                ],
                axis=1,
            ),
        ...
        f = lambda v: ssr2(
            v, x1.values, x2.values, y.values, yl.values, weight_method1, weight_method2
        )
        jac = lambda v: jacobian2(
            v, x1.values, x2.values, y.values, yl.values, weight_method1, weight_method2
        )
        ...
        f = lambda v: ssr2(
            v, x1.values, x2.values, y.values, yl, weight_method1, weight_method2
        )
        jac = lambda v: jacobian2(
            v, x1.values, x2.values, y.values, yl, weight_method1, weight_method2
        )
        ...
        opt_res = least_squares(
            f,
            np.concatenate(
                [c[0:3], weight_method1.init_params(), weight_method2.init_params()]
            ),
            jac,
            xtol=1e-9,
            ftol=1e-9,
...
def midas_adl(
    ...
    methods = {"fixed": fixed_window, "rolling": rolling, "recursive": recursive}

    return methods[method](
        y_in, x_in, start_date, end_date, xlag, ylag, horizon, forecast_horizon, poly
    )
...
def fixed_window(
    ...
    y, yl, x, yf, ylf, xf = mix_freq(
        y_in, x_in, xlag, ylag, horizon, start_date=start_date, end_date=end_date
    )
...
def rolling(
    ...
    return (
        rmse(preds, targets),
        pd.DataFrame(
            {"preds": preds, "targets": targets}, index=pd.DatetimeIndex(dt_index)
        ),
    )
...
def midas_adl2(
    ...
    methods = {"fixed": fixed_window2, "rolling": rolling2, "recursive": recursive2}
...
def fixed_window2(
    ...
    y, yl, x1, x2, yf, ylf, x1f, x2f = mix_freq2(
        y_in,
        x1_in,
        x2_in,
        x1lag,
